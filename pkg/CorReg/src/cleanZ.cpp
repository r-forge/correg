...
SEXP cleanZ(SEXP X,SEXP Z,SEXP bic_vide_vect,SEXP methode_BIC,SEXP plot,SEXP bla)
{
...
//somme a la main
  ...
  while(step<nbcand)  
  {
    list_cand.resize(compte-1,2);//le nombre de candidats est la compléxité du modèle
    compte=0;//initialisation du vecteur liste désigne le numéro du candidat
  //liste candidats (couples [i,j])
//    list_cand.resize(nbcand,2);//le nombre de candidats est la compléxité du modèle
    ...
    //modification (calcul du Z)    (Zcand avec methode (rejet ou relax)   Z,i,j,methode="relax", p2max=inf,rmax=inf)
      ...
      Zcand=newZ;
      Zcand(i_loc,j_loc)=1-Zcand(i_loc,j_loc);//modification de Z par rapport au candidat selectionné
