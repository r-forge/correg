...
mixture_generator<-function(n=130,
   ...
   if(is.null(R2)){
      if(is.null(sigma_X) & !gamma){#si R2 est nul et qu'on n'a pas de sigma X alors on le définit
         R2=0.99
      }
   }
   ...
   #       sd_vect=apply(X[,-1],2,sd)
   #       mean_vect=apply(X[,-1],2,mean)
   #       for(j in list_X2){
   #          B[1,j+1]=B[1,j+1]-mean_vect[j]
   #          B[,-1][,j]=B[,-1][,j]/sd_vect[j]
   #       }
   #       X=cbind(1,scale(X[,-1]))
   #    }
