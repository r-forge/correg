...
muiZ<-function(mixmod=mixmod,components=components,Z=Z,Zc=Zc,alpha=alpha,Ir=Ir,sigma_IR=sigma_IR,p=p){ 
   ...
   Sigma=Matrix(0,ncol=p,nrow=p)# à intialiser au début en creux symétrique et à passer en argument
   diag(Sigma)=sigmai
   for(j in Ir){
      for (i in (1:p)[-j]){
   ...
   return(list(mui=mui,sigmai=sigmai,Sigma=Sigma))#en fait sigmai ne sert à rien car stocké dans la diagonale de Sigma
}
#version C: laisser comp_vect pour éviter les recopie.