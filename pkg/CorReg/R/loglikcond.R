...
loglikcond<-function(X=X,mui=mui,Sigma=Sigma,M=M,i=i,Zc=Zc){ 
   res=mvtnorm::dmvnorm(x=X[i,],mean=mui,sigma=Sigma)#vraisemblance globale de l'individu
   ...
   if(length(quimank_i)>0){
      ...
      res=res/mvtnorm::dmvnorm(x=X[i,quimank_i],mean = mu,sigma=sigma)#on divise par la vraisemblance conditionnelle des manquants pour obtenir la marginale
