...
cleanZtest<-function (Z = Z, X = X, pvalmin = 0.05, methode = 1, adj = T,global=F,bonferroni=F) 
{
  if(global){
    ...
    for (i in quicol) {
      ...
      if (global_pval>pvalmin) {#équation pourrie, on la supprime
        Z[, i] = 0
      }
    }
  }else{#on regarde chaque coef donc on boucle jusqu'à stabilité
    pvalminini=pvalmin
    change=(colSums(Z) != 0)#colonnes à regarder
    quicol=which(change)
    while(length(quicol)>0 ){
      ...
      for (i in quicol) {#nettoyage
        ...
        #on élague juste les coefs pourris
        if(length(Z[qui,i][coefs_pval[-1]>pvalmin])>0){#on a des choses à changer
          Z[qui,i][coefs_pval[-1]>pvalmin]=0#on supprime
        }else{#rien n'a changé donc on marque la colonne
          change[i]=F
        }         
      }
      quicol=which(change & (colSums(Z) != 0))#on doit revérifier les colonnes modifiées encore non nulles
    }
  }
