...
First, we observe that if each variable in $\boldsymbol{X}_r$ is a Gaussian mixture, then there is at least one Gaussian mixture on the right of each sub-regression. So there is a non-empty subset of $\boldsymbol{X} \subset \boldsymbol{X}_f$ containing Gaussian mixtures. We note $\boldsymbol{X}_{S^c}$ its complement (empty or not) in $\boldsymbol{X}_f$


The theorem from Maugis guarantee that a sub-regression between Gaussian mixtures is identifiable.
		For each sub-regression, 
