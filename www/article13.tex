...
%In table \ref{compZvrai} 
We compare found structures in different contexts with both Uniform and Hierarchical Uniform {\it a priori} law on $P(S)$, noted respectively $BIC$ and $BIC_+$. 
We see that usage of $BIC_+$ gives sparser models even with $\max (\hat{p}_f^j)=5$. Moreover, this constraint is not active because observed complexities are smaller (this constraint only serves to accelerate the walk by reducing the dimension of $\mathcal{S}$ because each configuration was computed a hundred times), meaning that the stronger penalty implied by $BIC_+$ really is efficient. The datasets used for $BIC$ and $BIC_+$ are the same to keep the comparison  meaningful. %These datasets and $\hat{S}$ are used for   %tables \ref{YXlinOLS} to \ref{YX2linstep}.
...
\hline
\end{tabular} 
\caption{Results of the Markov chain: Mean observed and standard deviation (sd). } \label{compZvrai}
\end{table}
\clearpage
...
\begin{figure}[h!]
	\begin{minipage}[l]{.48\linewidth}
			\includegraphics[height=180px,width=245px]{figures/res_article/bon_gauche.png} 
			\caption{Number of $\boldsymbol{X_r}$ covariates found with both $BIC$ (dotted) and $BIC_+$ (plain)}
	\end{minipage} \
   \begin{minipage}[r]{.48\linewidth}
			\includegraphics[height=180px,width=240px]{figures/res_article/faux_gauche.png} 
			\caption{Number of covariates wrongly set in $\boldsymbol{X}_r$ with both $BIC$ (dotted) and $BIC_+$ (plain)} 
   \end{minipage}
\end{figure}
