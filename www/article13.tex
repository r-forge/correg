...
We have $I_f=\{I_f^1,\dots,I_f^p \}$ the set of the sets of indices of exogenous covariates (explaining ones $=\boldsymbol{X}_f$) with $\forall j \notin I_r, I_f^j=\emptyset$. Then we have an explicit structure $S=(I_f,I_r,p_f,p_r)$ where $\boldsymbol{p_f}=(p_f^1,\dots,p_f^{p_r})$ is the vector of the number of covariates in each internal regression. Thus we have $p_r=|I_r|$ and $p_f^j=|I_f^j|$ where $|.|$ represents the cardinal of an ensemble.
...
\begin{eqnarray}
	...
	&=&\boldsymbol{X}_f\boldsymbol{\beta}_f^*+\boldsymbol{\varepsilon}_Y^*=\boldsymbol{X}\boldsymbol{\beta}^*+\boldsymbol{\varepsilon}_Y^* \textrm{ where }\boldsymbol{\beta}^*=(\boldsymbol{\beta}_f^*,\boldsymbol{\beta}_2^*) \textrm{ and } \hat{\boldsymbol{\beta}}_r^*=\boldsymbol{\beta}_r^*=\boldsymbol{0}
\end{eqnarray}
...
Moreover, the uncrossing constraint ($I_1\cap I_2=\emptyset$) guarantee to keep a simple structure easily interpretable (no cycles and no chain-effect) and straightforward readable.
	...
	\end{equation}			
		So we have the \textsc{OLS} unbiased estimator of $\boldsymbol{\beta}^*$: 
		\begin{equation}
