...
Moreover, the uncrossing constraint ($I_1\cap I_2=\emptyset$) guarantee to keep a simple structure easily interpretable (no cycles and no chain-effect) and straightforward readable.
	...
	\begin{eqnarray}
		\textsc{MSE}(\boldsymbol{\hat{\beta}}|\boldsymbol{\beta},\boldsymbol{X})&=&\operatorname{Bias}^2+\operatorname{Tr}(\operatorname{Var}(\boldsymbol{\hat{\beta}})) \\
			&=&0+\sigma_Y^2 \operatorname{Tr}((\boldsymbol{X}'\boldsymbol{X})^{-1}) \textrm{ for OLS and } 
	\end{eqnarray}	 
	To better illustrate the bias-variance tradeoff, we look at a simple example with $p=3$ variables. $X_1$ is composed by two independent scaled Gaussian $\mathcal{N}(0,1)$, $X_3=x_1+x_2+\varepsilon_3$ where $\varepsilon_3\sim{\mathcal{N}(0,\sigma_3^2)}$. We also have $\beta=(1,1,1)$ and $\sigma_Y=10$. Then we observe the theoretical Mean Squared Error (MSE) of the estimator of both OLS and \textsc{CorReg} model for several values of $\sigma_3$ (strength of the sub-regression) and $n$. Figures \ref{MQE1} to \ref{MQE3} show the theoretical MSE evolution with the strength of the sub-regression:
	\begin{equation}
