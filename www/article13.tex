...
\subsection{Estimator and properties}
	...
	\subsection{Advantages for interpretation}
		...
		At the opposite, methods like the LASSO with LAR will only keep one covariate for each group of correlated covariates  so we get $\hat{Y}=2aX_1$ and think that $X_2 \perp Y$ but $Y$ depends on $X_2$ so it is worse than grouping effect for interpretation.
			...
			Moreover, the uncrossing constraint ($I_1\cap I_2=\emptyset$) guarantee to keep a simple structure easily interpretable (no cycles and no chain-effect).
...
& & & cpl (sd) & 30 (0) & 13.85 (0.9) & 9.27 (0.5) \\
 &  & HU & MSE (sd) & 178323.95 (1426610.2) & 330.31 (134.8) & 141.03 (27.8) \\
& & & MSE (sd) & 178323.95 (1426610.2) & 330.31 (134.8) & 141.03 (27.8) \\
\hline
\hline %global50p40ratio0BICTRUEBICstarTRUEp1max5lambda5100Amaxp2max100sigma_sousreg0.001maxit9000maxtry20max_compl1scaleTRUEcandidates-1_13mars2014_02_25_40.csv
...
\hline
\end{tabular} 
\caption{Elasticnet combined with constrained \textsc{CorReg}.$Y$  depends on all variables in $X$. \textsc{CorReg} logically wins}\label{YXlinenet}
\end{table}
...
\hline
\end{tabular} 
\caption{OLS and OLS combined with constrained \textsc{CorReg}.$Y$  depends on all variables in $X_2$. } \label{YX2linOLS}
\end{table}
...
\hline
\end{tabular} 
\caption{ $Y$ depends on all variables in $X$. \textsc{CorReg} is not too far from LASSO.} \label{resYnonlin}
\end{table}
