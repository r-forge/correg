...
%		I_1^j &=&\{ \textrm{ indices des covariables qui expliquent $X^j$} \} \\ % \{i |Z_{i,j}=1 \} 
		I_2&=&\{j |\sharp I_1^j>0 \}  \textrm{ indices des variables dépendantes} \\
		p_2&=& \sharp I_2 \\
		p_1&=&(p_1^1,\dots,p_1^{p_2}) \textrm{ avec }p_1^j=\sharp I_1^j 
	\end{eqnarray}
%	Où $Z$ est la matrice d'adjacence du graphe qui définit les dépendances entres variables (SEM récursif).
...
\section{Recherche de structure}
	...
	%	\subsection{Pénalisation de la vraisemblance}
		Le choix de structure s'appuie sur $BIC^*$, vraisemblance pénalisée de la structure à la manière du critère BIC~\cite{BIChuard}. 
		\begin{eqnarray}
...
%	\subsection{Parcours Markovien de saut}
	...
	A chaque étape de l'algorithme MCMC, pour $S \in \mathcal{S}$ (ensemble des structures réalisables) on définit un voisinage $\mathcal{V}_{S,j}$ avec $j \sim \mathcal{U}(\{1,\dots,p\}) $ :% de la manière suivante  :	
	\begin{eqnarray}
	...
	$S$ est entièrement défini à partir de $I_1$ donc on se contente ici de décrire les modifications dans $I_1$. %(les autres éléments de $S$ seront ensuite recalculés selon leur définition).
