...
\section{Décorrélation par modèle génératif}
	...
	\begin{eqnarray}
	Y_{|X,S}&=&XA+\varepsilon_Y= X^{I_2^c}A_{1}+X^{I_2}A_{2}+\varepsilon_Y \textrm{ avec } \varepsilon_Y \sim \mathcal{N}(0,\sigma_Y^2) \label{MainR}\\
...
%	\subsection{Parcours Markovien de saut}
	...
	A chaque étape de l'algorithme MCMC, pour $S \in \mathcal{S}$ (ensemble des structures réalisables) on définit un voisinage $\mathcal{V}_{S}$ de $p$ candidats , mais le package \textsc{CorReg} permet à l'utilisateur de modifier ce voisinage.
