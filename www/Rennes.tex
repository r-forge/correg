...
\section{Estimateur}
	Les Moindres Carrés Ordinaires (MCO) donnent (maximum de vraisemblance): 
		...
		CorReg réduit la variance de en estimant $Y$ seulement avec $X^{I_1}$, sachant (\ref{SR}) et (\ref{Trueexpl}).
			\begin{equation}
...
%	\subsection{Parcours Markovien de saut}
	...
	où $\Delta BIC(S,\mathcal{V}_{S,j})=BIC(S)-\min\{BIC(\tilde{S})| \tilde{S} \in \mathcal{V}_{S,j} \} $.
	\newline
	
	 Ce qui donne enfin $\forall (S,\tilde{S}) \in \mathcal{S}^2 $ :
		\begin{displaymath}
