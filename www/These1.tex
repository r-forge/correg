...
\chapter{Missing values}
	...
	\section{How to manage missing values in the MCMC ?}
		...
		In the MCMC we need to compute the likelihood of the structure. When missing values occurs, we restrict the likelihood to the known values according to equation (\ref{integralmiss}). we have
		\begin{eqnarray}
			...
			&=&\prod_{i=1}^n\left[ g(\boldsymbol{X}_i^{I_r} |\boldsymbol{X}_i^{I_f},\Theta)\prod_{\substack{j \notin I_r \\ \boldsymbol{M}_{i,j}=0}} f(x_{i,j}|\Theta) \right]
		\end{eqnarray}
		reminding that individuals are $iid$ and covariates in $\boldsymbol{X}_f$ are orthogonal.  Residuals of the sub-regressions are orthogonal but missing values can make the residuals dependent. We have to decompose more precisely $g(\boldsymbol{X}_i^{I_r} |\boldsymbol{X}_i^{I_f},\Theta)$. To have a better view on the dependencies implied, we first write the marginal distributions. $\forall (i,j) \textrm{ with } \boldsymbol{M}_{i,j}\neq 0 \textrm{ and } j \in I_r $:
		\begin{eqnarray}
g(x_{i,j}| \boldsymbol{X}_{i}^{I_f},\Theta)=g(x_{i,j}| \boldsymbol{X}_{i}^{I_f^j},\Theta) &=& \sum_{k=1}^{k_{ij}}\pi_{ij,k}\Phi(x_{i,j}|\mu_{ij,k},\Sigma_{ij,k}) \textrm{ where }  \label{Missingdensity}\\
				...
				\boldsymbol{\Sigma}_{ij} &=& \sigma_j^2 + \bigoplus_{\substack{l \in I_f^j \\ \boldsymbol{M}_{i,l}=1 }}\alpha_{i,l}^2 \boldsymbol{\Sigma}_l		\end{eqnarray}		 
		

		
		
		
		Each value is seen as different random variable noted $x_{i,j}$ for the $i^{th}$ value of $\boldsymbol{X}^j$. But the structure itself makes things more complicated because known values are not all $iid$ (if $j \in I_r$ ).
