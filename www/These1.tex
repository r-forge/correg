...
\end{displaymath}
where $I_r^j=\{i \in I_r| j \in I_f^j \}$ So we will make imputations (for E step only) according to $P(\boldsymbol{X}^j|\boldsymbol{X}_O^{I_r^j})$
		\\
...
\begin{eqnarray}
	P(x_{i,j},\boldsymbol{X}_{i,O}^{I_r^j})&=&\sum_{k \in K_{ij}} \pi_{ij,k} \phi(x_{i,j},\boldsymbol{X}_{i,O}^{I_r^j},\boldsymbol{\mu}_{ij,k},\boldsymbol{\Sigma}_{ij,k}) \textrm{ where } \\
	\pi_{ij}&=& 
\end{eqnarray}
...
Another way is to only use the structure for $\boldsymbol{X}^{I_r}$ and use the distribution given by Mixmod for $\boldsymbol{X}^{I_f}$.
