...
with	  $\forall (i,j) \textrm{ with } \boldsymbol{M}_{i,j}= 0 \textrm{ and } j \notin I_r $:
	 \begin{equation}
	 f(x_{i,j};\boldsymbol{\alpha},\Theta,S)=\sum_{k=1}^{K_j}\pi_{j,k}\Phi_k(x_{i,j};\mu_{j,k},\Sigma_{j,k})
...
\begin{eqnarray}
		f(\boldsymbol{X}^{I_r}_{i,O}|\boldsymbol{X}^{I_f}_{i,O};\boldsymbol{\alpha},\Theta,S)&=&\sum_{k=1}^{K_{i,I_r}}\pi_{i,I_r,k}\Phi_k(\boldsymbol{X}^{I_r}_{i,O}|\boldsymbol{X}^{I_f}_{i,O};\boldsymbol{\mu}_{\boldsymbol{X}^{I_r}_{i,O}|\boldsymbol{X}^{I_f}_{i,O},k},\boldsymbol{\Sigma}_{\boldsymbol{X}^{I_r}_{i,O}|\boldsymbol{X}^{I_f}_{i,O},k})\\
		\boldsymbol{\pi}_{i,I_r}&=& \bigotimes_{\substack{j \in I_f \\M_{i,j}=1}}\boldsymbol{\pi}_{j} \textrm{ (Kronecker product) } \\
		K_{i,I_r}&=&|\boldsymbol{\pi}_{i,I_r}| \textrm{ and } \forall 1\leq k\leq K_{i,I_r}  \\
		\boldsymbol{\mu}_{\boldsymbol{X}^{I_r}_{i,O}|\boldsymbol{X}^{I_f}_{i,O},k}&=& \boldsymbol{\mu}_{i,I_r,k}+\boldsymbol{\Sigma}_{X_{i,O}^{I_r},X_{i,O}^{I_f},k}(\boldsymbol{\Sigma}_{X_{i,O}^{I_f},X_{i,O}^{I_f},k})^{-1}( ^t\boldsymbol{X}_{i,O}^{I_f}-\boldsymbol{\mu}_{X_{i,O}^{I_f},k})\\
		\boldsymbol{\Sigma}_{\boldsymbol{X}^{I_r}_{i,O}|\boldsymbol{X}^{I_f}_{i,O},k}&=& \\
		\boldsymbol{\mu}_{X_{i,O}^{I_f}}&=&  \prod_{\substack{j \in I_f \\M_{i,j}=0}}\boldsymbol{\mu}_{j} \textrm{ (Cartesian product) } \\	
\end{eqnarray}		
