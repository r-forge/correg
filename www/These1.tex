...
\chapter{Missing values}
	...
	We note $\boldsymbol{X}_M$ the missing values and $\boldsymbol{X}_{\bar M}$. $\Theta$ stands for the parameters of the Gaussian mixture followed by $\boldsymbol{X}$.
...
%	
		...
		\subsection{Estimation of the coefficients in each regression}
			...
			The EM algorithm can be written here: we start with some $\Theta^{(0)} $ initial value of $\Theta$. The $\pi_{ij,k}$ are estimated once for each covariate (by Mixmod) and stay the same during the EM algorithm.
			E step : estimation of 
			\begin{equation}
				\boldsymbol{X}^{(h)}=E(\boldsymbol{X}|\boldsymbol{X}_{\bar M},\Theta^{(h)})
			\end{equation}
			$\forall (i,j), \boldsymbol{M}_{i,j}=1$, 			
			\begin{equation}
				\boldsymbol{X}^{(h)_{i,j}}=E(\boldsymbol{X}_{i,j}|\boldsymbol{X}_{\bar M},\Theta^{(h)})=\sum_{k=1}^{k_{ij}}\pi_{ij,k}\mu_{ij,k}^{(h)} \label{Estep}
			\end{equation}
