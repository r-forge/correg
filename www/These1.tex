...
\section{Graph theory}
	...
	\begin{eqnarray}
		...
		P(S)&=&P(I_f , \boldsymbol{p}_f|I_r,p_r)P(I_r,p_r) \\
		...
		P(I_f|I_r,p_r)&=& \frac{1}{(2^{p-pr}-1)^{p_r}}\\
		|\mathcal{S}_p|&=&\sum_{p_r=0}^{p-1}{p \choose p_r}(2^{p-pr}-1)^{p_r}
	\end{eqnarray}
	where \begin{equation}
	{n \choose k}=\frac{n!}{k!(n-k)!}
	\end{equation} is the binomial coefficient.\\
	We have then $|\mathcal{S}_2| =3 $,$|\mathcal{S}_3| =13 $ and $|\mathcal{S}_10| >13.26\times10^9 $ so the number of feasible structures really explodes when $p$ is growing.
...
De plus le graphical Lasso ne fournit qu'un graphe non-orienté. Il faut alors définir une orientation qui corresponde à un graphe bi-parti $(\boldsymbol{X}_r,\boldsymbol{X}_f)$, ce qui revient à un algorithme de 2-coloration. De plus, trouver une orientation bi-parti nécessite l'absence de cycles de longueur impair (ce que ne garantit pas le graphical lasso).
		Glasso est donc intéressant mais demanderait l'hypothèse de normalité et un algorithme d'orientation, le tout sans garantie d'aboutir à une structure (à moins de rompre les cycles impairs selon une heuristique par suppression d'un arc dans chaque cycle impair)
	\section{CorReg}	
