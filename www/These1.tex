...
\begin{eqnarray}
		f(\boldsymbol{X}^{I_r}_{i,O}|\boldsymbol{X}^{I_f}_{i,O};\boldsymbol{\alpha},\Theta,S)&=&\sum_{k=1}^{K_{i,I_r}}\pi_{i,I_r,k}\\
		\boldsymbol{\pi}_{i,I_r}&=& \bigotimes_{\substack{j \in I_f \\M_{i,j}=1}}\boldsymbol{\pi}_{j}
\end{eqnarray}		
