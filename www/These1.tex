...
\chapter{Missing values}
	Missing values are a very recurrent issue in industry. We note $\boldsymbol{M}$ the binary matrix indicating whereas a value is missing (1) or not (0).
	\section{How to manage missing values in the MCMC ?}
		...
		In the MCMC we need to compute the likelihood of the structure. When missing values occurs, we restrict the likelihood to the known values. So we look at each non-missing value separately. Each value is seen as different random variable noted $x_{i,j}$.  But the structure itself makes things more complicated because known values are not all $iid$ (if $j \in I_r$ ).
			...
			\begin{itemize}
				\item if $\boldsymbol{M}_{i,j}=1$ : $x_{i,j}$ is not considered because we restrict the likelihood to known values.
				\item else if $j \in I_f$ : like in previous method, we use the density estimated  ($e.g.$ a Gaussian Mixture model estimated by \textsc{Mixmod}) for $\boldsymbol{X}^j$. The $n$ random variables in $\boldsymbol{X}^j$ are $iid$.
				\item else :
			\end{itemize}
			\begin{eqnarray}
				x_{i,j}| \boldsymbol{X}_{i,I_f^j} &\sim & \sum_{\substack{1\leq k \leq p \\ \boldsymbol{\alpha}_k^j\neq 0 } } x_{i,k}\boldsymbol{\alpha}_k^j  + \mathcal{N}(0;\sigma_j) \\
				&=& \sum_{\substack{1\leq k \leq p \\ \boldsymbol{\alpha}_k^j\neq 0 \\ \boldsymbol{M}_{i,k}\neq 0 } } x_{i,k}\boldsymbol{\alpha}_k^j  + \mathcal{N}(\sum_{\substack{1\leq k \leq p \\ \boldsymbol{\alpha}_k^j\neq 0 \\ \boldsymbol{M}_{i,k}= 0 } } x_{i,k}\boldsymbol{\alpha}_k^j \ ; \ \sigma_j) \label{Missingdensity}\\ 
			\end{eqnarray}
